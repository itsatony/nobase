 ...
 *
 */
aNoBase.prototype.add = function(document, opts, callback) {
	var thisNodeBase = this;
	var options = (typeof opts === 'object') ? opts : { overwrite: false };
	if (typeof options.overwrite !== 'boolean') options.overwrite = false;
	// if (typeof options.noresult !== 'boolean') options.noresult = true;
	var errors = null;
	function doAdd(doc) {
		doc._id = thisNodeBase.nbid(doc._id);
		if (typeof thisNodeBase.store[doc._id] === 'undefined') thisNodeBase.numberOfEntries++;
		if (typeof thisNodeBase.store[doc._id] === 'undefined' || options.overwrite === true) {
			thisNodeBase.store[doc._id] = doc;
	...
	for (var y = document.length-1; y > -1; y--) {
		doAdd(document[y]);
	}
	/* 	
	} else {
