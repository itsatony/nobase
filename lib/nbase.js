...
Nbase.prototype.uid = function() {
	this.insertCount += 1;
	var baseId = Date.now() + 'x' + this.insertCount;
	return baseId;
};
...
Nbase.prototype.addItem = function(item, callback) {
	...
	if (typeof item[thisNbase.uuidName] === 'undefined') {
		item[thisNbase.uuidName] = this.uid();
	}
...
Nbase.prototype._removeByIndex = function(indexArray, limit, skip, callback) {
	...
	for (var i=0; i<indexArray.length; i++) {
		...
		var toDelete = thisNbase._get(i);
		var keys = thisNbase.index[i];
		for (var n = 0; n < keys.length; n++) {
			delete thisNbase.store[keys[n]][i];
			thisNbase.entryCount+=-1;
		}
		delete thisNbase.index[i];
	}
	if (typeof callback === 'function') {
		...
		function(err, res) {
			...
			if (err !== null) {
				...
				for (var n=0; n < res.length; n++) {
					indexArray.push(res[n][thisNbase.uuidName]);
				}
			}
...
Nbase.prototype.find = function(query, opts, callback) {
	...
	if (typeof opts === 'function') {
		...
		var options = minions.defaultTo(opts, { skip:0, limit: false, fields:[] });
	}
	var limit = (typeof options.limit === 'number') ? options.limit : false;
	var skip = (typeof options.skip === 'number') ? options.skip : 0;
